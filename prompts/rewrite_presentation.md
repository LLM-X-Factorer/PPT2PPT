...
**字数硬约束**:
- `lane.outcome` 单段 ≤ **8 中文字**,三段总长 ≤ **26 字**(箭头不计)
  ...
  - 例:标准·产品 phase 2 "V7 正式发布 · 用户教育" + 标准·工具 phase 2 "AI 助手正式上线 · 接管 70% 答疑"
    - 合并 = "V7 发布 + AI 接管 70% 答疑"? 18 字,溢出
    - 必须再裁:取主语并各保留一个最关键成果 → "正式发布 → AI 接管 70% 答疑" 14 字 ≤ 8 字×2,两段算一段时仍可接受;但通常拆分到两段更安全 — phase 2 单段就用 "AI 接管 70% 答疑"(9 字,边界),或更精简 "AI 70% 答疑"
