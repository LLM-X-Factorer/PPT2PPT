...
**核心约束**:演讲面 MILESTONE.title 的冒号子句**必须呼应 3 lane 名(不是 5 group 名)**。当 v1 Q2.title 的冒号子句已经是 3 元素时,直接复用;当 v1 Q2.title 是 5 元素(因为 v1 Q2 有 5 group)时,**必须改写为 3 元素**,与 v2 lane 数对齐。

| v1 Q2.title 形态 | v2 MILESTONE.title 处理 |
|---|---|
| 3 元素冒号子句(如 demo "Q2 三件事打透:Onboarding · V7 · AI 辅助") | 直接复用 |
| 5 元素冒号子句(如真实 "Q2 必打五件事:GEO + V7 + AI 助教 + 沙龙 + 文创") | **必须改 3 元素**,呼应 3 lane 命名(如 "Q2 必打三事:引流 + 爆品 + 价值") |
| "Q2 计划"(无冒号子句) | 加 3 元素冒号子句:"Q2 跑通 X · Y · Z" |

**根因**:演讲面 MILESTONE 是 3 lane 卡片,title 数值与 lane 数同构是叙事一致性要求;且 5 元素冒号子句在 40pt 大字版式视觉总长溢出,会换行截断。

**事实层一致性**:title 改 3 元素**不算"事实层失准"** — title 是叙事概括(数"三事/三链路"是 lane 命名层面),具体的 5 个 group 内容仍在 v1 详情面 Q2.title 和 lane.task 里完整保留。
...
- **冒号子句紧凑性硬约束**(40pt 大字版式视觉单行):
  - 单元素 ≤ **3 中文字**(如 "双 IP" 2 字、"高客单" 3 字、"运营闭环" 4 字属边界,5 字+必换行)
  - 整体冒号子句去除分隔符后 ≤ **12 字符**(中文/英文/数字均计入)
  - 分隔符优先级:` · `(中点+空格,最紧凑) > ` + `(加号+空格,次之)。混用要慎重,加号本身视觉占位比中点宽
  - 元素超 3 字时**必须缩写**:如真实数据 "IP 引流"(4 字符) → "IP" 或 "引流";"运营闭环"(4 字) → "闭环"
- 例:
  - ✅ "Q2 三件事打透:Onboarding · V7 · AI 辅助"(5 字 + 3 元素 · 单元素 ≤ 4 字符)
  - ✅ "Q2 三事并进:Onboarding · V7 · AI 辅助"(4 字 + 3 元素)
  - ✅ "Q2 必跑三链路:双 IP · 高客单 · 闭环"(5 字 + 3 元素 · 各 2/3/2 字)
  ...
  - ❌ "Q2 必跑三链路:IP 引流 + 高客单 + 运营闭环"(5 字 + 3 元素,但单元素 4 字 + 加号分隔,视觉总长溢出)→ 改为 "Q2 必跑三链路:双 IP · 高客单 · 闭环"(单元素压到 ≤ 3 字 + 中点分隔)
  - ❌ "Q2 必打五件事:GEO + V7 + AI 助教 + 沙龙 + 文创"(5 字 + **5 元素冒号子句**)→ 按规则 3 改为 3 元素 "Q2 必打三事:引流 + 爆品 + 价值"
